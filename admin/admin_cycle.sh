...
#  if ((ETIMEdoy % 5 == 1 && ETIMEh == 0)); then
  ...
  echo "$now [DONE] $TIMEf - Download and remove remote files (background job completed)" >> $logfile
}
...
if [ ! -s "$obsdir/${ETIMEf2}/obs_${ETIMEf}.dat" ]; then
  echo "$now [WAIT] $TIMEf - Observation files are not ready." >> $logfile
  rm -f $lockfile
  exit
fi
...
if ((success == 1)); then
  now="$(date -u +'%Y-%m-%d %H:%M:%S')"
  echo "$now [TRAN] $TIMEf - Download and remove remote files (background job)" >> $logfile
