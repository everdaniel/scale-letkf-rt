...
#    fi
    ...
    ssh ${r_url} "rm -r ${r_gradsdir_da}/${istimef}"
...
iatime="$(date -ud "$LCYCLE second $istime" +'%Y-%m-%d %H:%M:%S')"
iatimef="$(date -ud "$iatime" +'%Y%m%d%H%M%S')"
iaatimef=$(date -ud "$LCYCLE second $iatime" +'%Y%m%d%H%M%S')
...
while ((istimef <= ETIMEf)); do
  ...
  ln -s $gradsdir/$(date -ud "$istime" +'%Y%m%d%H')/atm_${istimef}.grd $tmpdir/${istimef}/mean
  ln -s $gradsdir/$(date -ud "$iatime" +'%Y%m%d%H')/atm_${iatimef}.grd $tmpdir/${istimef}/mean
  ln -s $gradsdir/$(date -ud "$iatime" +'%Y%m%d%H')/atm_${iaatimef}.grd $tmpdir/${istimef}/mean
  ln -s $gradsdir/$(date -ud "$istime" +'%Y%m%d%H')/sfc_${istimef}.grd $tmpdir/${istimef}/mean
  ln -s $gradsdir/$(date -ud "$iatime" +'%Y%m%d%H')/sfc_${iatimef}.grd $tmpdir/${istimef}/mean
  ln -s $gradsdir/$(date -ud "$iatime" +'%Y%m%d%H')/sfc_${iaatimef}.grd $tmpdir/${istimef}/mean
  ln -s $gradsdir/$(date -ud "$istime" +'%Y%m%d%H')/land_${istimef}.grd $tmpdir/${istimef}/mean
  ln -s $gradsdir/$(date -ud "$iatime" +'%Y%m%d%H')/land_${iatimef}.grd $tmpdir/${istimef}/mean
  ln -s $gradsdir/$(date -ud "$iatime" +'%Y%m%d%H')/land_${iaatimef}.grd $tmpdir/${istimef}/mean
  rsync -avL $tmpdir/${istimef} ${r_url}:${r_gradsdir_da} >> $outfile
