...
outdir="$(cd "$wkdir/../exp/test_5.0.0_d1" && pwd)"
web_fmdir="bowmore:/home/gylien/public_html/scale/data/test_5.0.0"
...
for ith in $(seq $threads); do
  ...
  grads -blc "plot_driver_1h.gs $outdir/${TIMEf}/ctl/fcstgp_mean.ctl $itstart $itend 1" \
        > /dev/null 2>&1 &
...
#      mv -f $plotdir/out/${prefix}_f${itfcstf}.png $outdir/${TIMEf}/fcstgpi/mean/${prefix}
      mv -f $plotdir/out/${prefix}_f${itfcstf}.png $wkdir/results/${TIMEf}
    else
      alldone=0
    fi
    if [ -s "$plotdir/out/${prefix}_f${itfcstf}.eps" ]; then
#      mv -f $plotdir/out/${prefix}_f${itfcstf}.eps $outdir/${TIMEf}/fcstgpi/mean/${prefix}
      mv -f $plotdir/out/${prefix}_f${itfcstf}.eps $wkdir/results/${TIMEf}
    else
      alldone=0
    fi
  done
