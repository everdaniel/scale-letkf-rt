...
if [ ! -s "prepbufr.$YYYYMMDDHH" ]; then
  rm -f gfs.t${HH}z.prepbufr.nr
#  wget --limit-rate=500k ftp://ftp.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.${YYYY}${MM}${DD}${HH}/gfs.t${HH}z.adpupa.tm00.bufr_d.unblok
  wget --cache=off http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.${YYYY}${MM}${DD}${HH}/gfs.t${HH}z.prepbufr.nr
  if [ -s "gfs.t${HH}z.prepbufr.nr" ]; then
    mv -f gfs.t${HH}z.prepbufr.nr prepbufr.$YYYYMMDDHH
    now=`date -u +'%Y-%m-%d %H:%M:%S'`
...
#  rm -f gfs.t${HH}z.prepbufr.unblok.nr
##  wget --limit-rate=500k ftp://ftp.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.${YYYY}${MM}${DD}${HH}/gfs.t${HH}z.adpupa.tm00.bufr_d.unblok
#  wget --cache=off http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs.${YYYY}${MM}${DD}${HH}/gfs.t${HH}z.prepbufr.unblok.nr
#  if [ -s "gfs.t${HH}z.prepbufr.unblok.nr" ]; then
#    mv -f gfs.t${HH}z.prepbufr.unblok.nr prepbufr.$YYYYMMDDHH.unblok
#    now=`date -u +'%Y-%m-%d %H:%M:%S'`
...
if [ "$allget" -eq 1 ]; then
  ...
  bash $wkdir/run/dec_prepbufr/convert.sh "${YYYY}-${MM}-${DD} ${HH}" "$wkdir/${YYYYMMDDHH}" "$wkdir/../ncepobs_letkf/${YYYYMMDDHH}" \
