...
if [ ! -s "prepbufr.$YYYYMMDDHH" ]; then
  rm -f gdas1.t${HH}z.prepbufr.nr
#  wget --limit-rate=500k ftp://ftp.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gdas.${YYYY}${MM}${DD}/gdas1.t${HH}z.adpupa.tm00.bufr_d.unblok
  wget --cache=off http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gdas.${YYYY}${MM}${DD}/gdas1.t${HH}z.prepbufr.nr
  if [ -s "gdas1.t${HH}z.prepbufr.nr" ]; then
    mv -f gdas1.t${HH}z.prepbufr.nr prepbufr.$YYYYMMDDHH
    now=`date -u +'%Y-%m-%d %H:%M:%S'`
...
#  rm -f gdas1.t${HH}z.prepbufr.unblok.nr
##  wget --limit-rate=500k ftp://ftp.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gdas.${YYYY}${MM}${DD}/gdas1.t${HH}z.adpupa.tm00.bufr_d.unblok
#  wget --cache=off http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gdas.${YYYY}${MM}${DD}/gdas1.t${HH}z.prepbufr.unblok.nr
#  if [ -s "gdas1.t${HH}z.prepbufr.unblok.nr" ]; then
#    mv -f gdas1.t${HH}z.prepbufr.unblok.nr prepbufr.$YYYYMMDDHH.unblok
#    now=`date -u +'%Y-%m-%d %H:%M:%S'`
...
if [ "$allget" -eq 1 ]; then
  ...
  bash $wkdir/run/dec_prepbufr/convert.sh "${YYYY}-${MM}-${DD} ${HH}" "$wkdir/${YYYYMMDDHH}" "$wkdir/../ncepobs_gdas_letkf/${YYYYMMDDHH}" \
